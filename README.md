...
<p align="justify">
Sotto turns your voice into text in the app you are using. Hold a shortcut,
speak, release, and keep working. Sotto transcribes on your Mac, cleans up the
result, and inserts it into the active field. If insertion is unavailable, it
...
| | What you get |
| --- | --- |
| **Fast dictation** | Hold <kbd>⌥</kbd> <kbd>Space</kbd> by default, speak, release, and keep moving. Toggle mode is available too. |
| **Out of the way** | A menu bar control and a quiet overlay show state without pulling you away. Light and dark themes, compact controls, and Reduce Motion support keep the interface calm. |
| **Your vocabulary** | Add replacements for names and terms, remove filler sounds, and normalize spacing and punctuation. |
| **Reliable insertion** | Sotto tries direct insertion first, then paste, and copies the result when macOS cannot insert it. |
| **Optional history** | Keep a bounded list of recent dictations, or turn history off. Audio recordings are temporary and are not stored in history. |
...
Sotto runs on Apple Silicon Macs with macOS 14 or newer. The speech model needs
roughly 500 MB of free disk space and is downloaded separately from the
application bundle.
...
To update or remove Sotto:

```sh
brew upgrade --cask sotto
brew uninstall --cask sotto
```

macOS may ask you to confirm **Open** the first time you launch Sotto.

### First launch

1. Open **Voice Engine** and install the speech engine.
...
4. Hold <kbd>⌥</kbd> <kbd>Space</kbd>, speak, and release. Turn off **Hold to
   dictate** in **Shortcuts** to use the same shortcut as a toggle.
...
<p align="justify">
Sotto stores preferences, vocabulary, optional history, temporary recordings, and
the downloaded speech model under <code>~/Library/Application Support/Sotto</code>.
